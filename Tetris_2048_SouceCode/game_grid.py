...
class GameGrid:
   ...
   def sumCheck(self, columnSet, current_tetromino):
      for x in columnSet:
         for y in range(self.grid_height - 1):
            tile_bottom = self.tile_matrix[y][x]
            tile_top = self.tile_matrix[y+1][x]

            if tile_bottom == None or tile_top == None:
               continue

            if tile_bottom.number == tile_top.number:
               tile_bottom.number += tile_top.number
               self.score += tile_top.number
               tile_bottom.background_color = tile_top.color_generator()
               self.tile_matrix[y+1][x] = None
               for temp in range(y+2,self.grid_height):
                  tileToMove = self.tile_matrix[temp][x]
                  if tileToMove == None:
                     continue
                  tileToMove.move(0,-1)
                  self.tile_matrix[temp-1][x] = tileToMove
                  self.tile_matrix[temp][x] = None
               self.sumCheck(columnSet,current_tetromino)
               return
   ...
   def rowCheck(self,rowSet):
      ...
      if temp == None:
         return
      for y in range(temp+1,self.grid_height):
         for x in range(self.grid_width):
            if self.tile_matrix[y][x] != None:
               self.tile_matrix[y][x].move(0,-1 * i)
               self.tile_matrix[y-i][x] = self.tile_matrix[y][x]
               self.tile_matrix[y][x] = None
